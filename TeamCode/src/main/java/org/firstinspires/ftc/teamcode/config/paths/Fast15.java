...
import static org.firstinspires.ftc.teamcode.config.FieldPoses.redGateOpen2;
...
//                .setNoDeceleration()
    ...
    public PathChain gateIntake() { // go to gate from intake1
        return f.pathBuilder()
                .addPath(new BezierLine(gateOpen, gate))
                .setBrakingStrength(gateIntakeBreakStrength)
                .setLinearHeadingInterpolation(gateOpen.getHeading(), gate.getHeading(), 0.3)
                .build();
    }
